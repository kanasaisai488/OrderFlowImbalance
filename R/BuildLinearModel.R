...
#	builds the linear model to be used by the strategy

source(file = 'BuildLinearData.R')

BuildLinearModel <- function(key,
							 data,
							 full.day = T,
							 morning = T,
							 open.int = F,
	...
	if (full.day == T) {
		...
		morning.data <- BuildLinearData(data, morning=T, open.int=open.int, delay=delay,
									    lags=lags, functions=functions)
		evening.data <- BuildLinearData(data, morning=F, open.int=open.int, delay=delay,
										lags=lags, functions=functions)

		dMid.Response <- c(morning.data$dMid.Response, evening.data$dMid.Response)
		VOI <- rbind(morning.data$VOI, evening.data$VOI)
		OIR <- rbind(morning.data$OIR, evening.data$OIR)
		time.secs <- c(morning.data$time.secs, evening.data$time.secs)
		mid.price <- c(morning.data$mid.price, evening.data$mid.price)
		spread <- c(morning.data$spread, evening.data$spread)
		AvgTrade.price <- c(morning.data$AvgTrade.price, evening.data$AvgTrade.price)
		MPB <- rbind(morning.data$MPB, evening.data$MPB)
		trading.data <- rbind(morning.data$data, evening.data$data)

	} else {

		trading.data <- BuildLinearData(data, morning = morning, open.int = open.int,
										delay = delay, lags = lags, functions=functions)
	...
	x[['B']] <- data.frame(y=dMid.Response, VOI=f.VOI(VOI)/spread,
							OIR=f.OIR(OIR)/spread, MPB=MPB[,1]/spread)
